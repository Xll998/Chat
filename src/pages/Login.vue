...
export default {
  ...
  methods: {
    ...
    //发送dispatch请求  actions 抓取成功或者失败的数据， 写入state, 同步localStorage返回回执信息->login
    //login 确定后续工作
    login(){      
      // axios({
      //   url: '/api/login',
      ...
      //       } else{
      //         this.errMesg = res.data.msg
      //       }
      //   }
        // res => this.$router.push('/user')
      // )


      this.$store.dispatch('UPDATE_USER', {username: this.username, password: this.password
      }).then(
        //抓取回执
        receipt => {          
          receipt.err === 0 ? this.$router.push('/user') : this.errMess = receipt.msg
        }
      )
    }
  }
